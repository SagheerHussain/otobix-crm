...
// Load Banners list
  Future<void> _loadBannersList() async {
    ...
    try {
      final response = await ApiService.get(endpoint: AppUrls.fetchBannersList);
...
// Toggle Banner Status
  Future<void> toggleBannerStatus(SellMyCarBannersModel banner) async {
    ...
    final newStatus = currentStatus == AppConstants.banners.active
        ? AppConstants.banners.inactive
...
// Delete Banner
  Future<void> deleteBanner(SellMyCarBannersModel banner) async {
    try {
      ...
      if (response.statusCode == 200) {
        // Remove locally
        if (banner.type == AppConstants.banners.header) {
  ...
  int getActiveHeaderBannersCount() {
    return headerBannersList
        .where((b) => b.status == AppConstants.banners.active)
        .length;
  }
